...
才进入合格池。精选层还要求综合分不低于85、RSI位于45至70、20日波动率受控且风险项
不超过1项，再按综合分选择最多 Top-N（默认10只）。基本面缺失不会自动通过。
...
Backtests 案例目录当前包含以下十二个案例。目录回归测试会逐项确认模板存在、生成的 Python
...
| A股趋势回撤PIT组合 | `ashare_trend_pullback_portfolio` | 动态组合 | 四选一指数PIT、复权因子、成交额、申万一级行业；C模型另需PIT财务 |
...
评估当时股票池。全部达标股票进入“合格”层；通过更严格精选质量门槛的股票再按综合分
取最多 `topN` 只并标记为“精选”。因此合格数少于 `topN` 时不会再自动把整个合格池复制为
精选。该模板为 `researchOnly`，不会提交订单、构建持仓或进入策略准入。

`ashare_trend_pullback_portfolio` 是独立的可交易模板。它在每周最后一个交易日收盘后，使用
原始 OHLC 与时点复权因子构造趋势、回撤、ATR 和相对强弱特征，下一交易日用 MOO 执行。
模型 A 只使用趋势回撤，B 增加指数及行业同业相对强弱，C 再增加公告后可见的质量约束。
默认持有18只、总仓位90%、单股6%、一级行业25%，市场状态缩放默认关闭。运行前平台会生成
带哈希的 `ashare-trend-pullback-input.json.gz`，2016-01-01 之前、行业覆盖不足或 C 模型缺少
PIT财务时直接阻断。
