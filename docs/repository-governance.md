...
- require status checks `Governance` and `Dependency Review`;
- require the branch to be up to date before merging.

`Governance` is intentionally small and always-on. `Dependency Review` blocks a pull request that introduces a dependency with a `high` or `critical` known vulnerability. CodeQL is initially advisory rather than a required status check so its scheduled/code-scanning rollout can stabilize independently and external fork PRs are not deadlocked.

GitHub documents that required approvals may be set to zero while still requiring the pull-request path, and that strict required status checks require the topic branch to be up to date before merge.
...
The connected automation surface may not be allowed to mutate Rulesets or repository merge settings. When server-side writes are unavailable, apply the policy in GitHub:

1. Open **Settings → Rules → Rulesets**.
2. Create the `Protect main` branch ruleset using the default branch target and the rules above.
3. Create the `Protect release tags` tag ruleset for `refs/tags/v*`.
4. Open **Settings → General → Pull Requests** and align merge methods, head-branch deletion and update-branch behavior with the table above.
5. Apply the Description and Topics from `.github/repository-metadata.yml` if they are not already synchronized.
6. Run the remote audit below.
...
2. `Dependency Review` — required immediately after its first successful PR run.
