...
- Include TuShare money-flow and ST-list endpoints in the default A-share refresh, bootstrap their initial trade-date history, and publish provider-shaped money-flow, suspension, limit-price, and ST partitions atomically to Bronze without replaying unchanged files.
...
- Revalidate the 2026-08-02 actual environment as audit iteration 3, recording deployed terminal-trust, Dataset Release, Golden Pair, capability, bounded-list, Paper-trust and PageEnvelope closures while retaining current release convergence, Paper 21-session, Walk-Forward certificate, notification/capacity observation and Browser acceptance blockers with a conservative 66/100 Level 5 fail score.
- Close the remaining actual-environment P1 notification and capacity code gaps with bounded jittered delivery retries, persistent dead-letter evidence and operator requeue, delivery-aware degraded health, container/process memory attribution and headroom SLOs, recyclable Celery children, and configurable Compose CPU/memory limits with safer demand-worker concurrency.
