...
def test_frontend_does_not_call_retired_research_client_members():
    ...
    for path in FRONTEND_SRC.rglob("*"):
        if path.suffix not in {".ts", ".tsx"}:
            continue
        for match in member_pattern.finditer(path.read_text(encoding="utf-8")):
            references.setdefault(match.group("member"), set()).add(str(path.relative_to(ROOT)))
