...
def qmt_to_platform_symbol(stock_code: str) -> str:
    try:
...
def platform_to_qmt_symbol(instrument: str) -> str:
    text = instrument.strip().upper()
    if len(text) < 3 or text[:2] not in {"SH", "SZ"} or not text[2:].isdigit():
        raise ValueError(f"unsupported platform instrument: {instrument!r}")
...
def _event_time(value: Any) -> str:
    """Convert QMT epoch seconds, milliseconds, or compact local time to UTC."""
...
class XtQuantReadOnlyClient:
    ...
    def query_quotes(self, instruments: Sequence[str]) -> Sequence[QmtQuote]:
        ...
        codes = [platform_to_qmt_symbol(item) for item in requested]
        with self._lock:
            self.ensure_connected()
            try:
